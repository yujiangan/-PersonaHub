...
import { executeTool, TOOL_HANDLERS } from "~/server/agent/dispatch";
import type { ToolContext } from "~/server/agent/dispatch";
import type { GitHubUser } from "~/shared/types";
import type { SSEEmitter } from "~/server/lib/sse";

vi.mock("~/server/agent/tools/get-profile");
vi.mock("~/server/agent/tools/get-repos");
vi.mock("~/server/agent/tools/get-events");
vi.mock("~/server/agent/tools/get-stars");
...
describe("executeTool", () => {
  ...
  it("should call correct handler for get_user_profile", async () => {
    const { getUserProfile } = await import("~/server/agent/tools/get-profile");
