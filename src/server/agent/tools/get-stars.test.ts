...
import { getUserStars } from './get-stars';
...
describe('getUserStars', () => {
  ...
  it('should transform multiple starred repos', async () => {
    const { fetchAllPages } = await import('./fetch-pages');
    vi.mocked(fetchAllPages).mockResolvedValue([
      { id: 1, name: 'repo1', full_name: 'u1/r1', description: 'd1', language: 'JS', topics: [], stargazers_count: 100 },
      { id: 2, name: 'repo2', full_name: 'u2/r2', description: 'd2', language: 'TS', topics: ['ts'], stargazers_count: 200 },
    ]);
